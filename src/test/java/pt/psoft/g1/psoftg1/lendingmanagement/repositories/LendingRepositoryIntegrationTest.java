...
/**
 * SUT: LendingRepository (Spring Data JPA + custom fragment)
 *
 * Methods exercised by the integration test:
 *  - save(..), delete(..)                        → inherited from JpaRepository
 *  - findByLendingNumber(String)                 → derived query
 *  - listByReaderNumberAndIsbn(String, String)   → custom impl
 *  - getCountFromCurrentYear()                   → custom impl
 *  - listOutstandingByReaderNumber(String)       → custom impl
 *  - getAverageDuration()                        → custom impl
 *  - getOverdue(Page)                            → custom impl (uses Page number/size)
 */
